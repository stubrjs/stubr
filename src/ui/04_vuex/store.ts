...
const mutations: MutationTree<RootState> = {
	...
	SET_ROUTE(state, routeConfiguration: RouteConfiguration) {
		const _foundRouteIndex = findIndex(state.routeConfigurations, (_routeConfiguration: RouteConfiguration): boolean => {
		...
		if (_foundRouteIndex >= 0) {
			...
			state.routeConfigurations = [...state.routeConfigurations, routeConfiguration];
		}
	},
	RESET_ROUTES(state) {
	...
	SET_LOG_ENTRY(state, logEntryLocal: LogEntryLocal) {
		const _foundLogEntryIndex = findIndex(state.logEntries, (logEntry: LogEntryLocal): boolean => {
			return logEntry.id == logEntryLocal.id;
		});

		if (_foundLogEntryIndex >= 0) {
			const _logEntriesClone: LogEntryLocal[] = clone(state.logEntries);
			_logEntriesClone[_foundLogEntryIndex] = { ...logEntryLocal, timestamp: state.logEntries[_foundLogEntryIndex].timestamp };
...
const actions: ActionTree<RootState, RootState> = {
	...
	toggleRouteInterception({ state, dispatch }, routeInterception: RouteInterception) {
		if (state.socket) {
			state.socket.emit(EventType.MARK_ROUTE_FOR_INTERCEPTIONS, routeInterception, (routeConfiguration: RouteConfiguration) => {
				dispatch('setRoute', routeConfiguration);
			});
		} else {
			console.warn('toggleRouteInterception() could not be executed since no socket being defined');
		}
	},
	resolveRouteInterception({ state }, resolveInterception: ResolveInterception) {
		if (state.socket) {
			state.socket.emit(EventType.RESOLVE_INTERCEPTION, resolveInterception);
		} else {
			console.warn('resolveRouteInterception() could not be executed since no socket being defined');
		}
	}
};
...
const getters: GetterTree<RootState, RootState> = {
	...
	numberOfInterceptionMarkers(state): number {
		let _counter = 0;
		state.routeConfigurations.forEach((routeConfiguration: RouteConfiguration) => {
