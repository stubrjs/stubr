    ...
    <div class="log-tile">
        <div class="head">
            <div class="head-left" style="display: flex; align-items: center;">
                ...
                <v-tag v-if="noRouteMatch" color="red">NO ROUTE MATCH</v-tag>
                <v-tag v-if="noScenarioMatch" color="red">NO CASE MATCH</v-tag>
                <v-tag v-if="intercepted" color="orange">INTERCEPTED</v-tag>
                <v-icon
                    v-if="caseMatch"
                    class="checkmark"
                    width="15px"
                    ...
                    color="green"
                />
                <div v-if="logEntry.group" class="group">
                    {{ logEntry.group }}:
                </div>
                <div v-if="logEntry.name" class="name">
                    {{ logEntry.name }}
                </div>
            </div>
            <div class="head-right">
                <span v-if="logEntry.delay" class="delay">
        ...
        <div class="body">
            ...
            <v-formatted-payload
                ...
                :status-code="logEntry.response.status"
            ></v-formatted-payload>
        </div>
    </div>
</template>
...
export default Vue.extend({
    ...
    computed: {
        ...
        noRouteMatch(): boolean {
            return (
                (this.logEntry.error &&
                    this.logEntry.error.code == 'NO_ROUTE_MATCH') ||
                false
            );
        },
        noScenarioMatch(): boolean {
            return (
                (this.logEntry.error &&
                    this.logEntry.error.code == 'NO_SCENARIO_MATCH') ||
                false
            );
        },
        intercepted(): boolean {
